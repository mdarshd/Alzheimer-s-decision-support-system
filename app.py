...
Desktop clinical-decision-support GUI for early Alzheimer's disease screening.

The app walks a user through two stages:
1. A clinical questionnaire, scored by a Random Forest model trained on
   tabular patient data (see prediction/csv_prediction.py).
2. An optional MRI upload, classified by a ResNet18 deep learning model
   into an impairment stage (see models/).

Each result is paired with a plain-language explanation generated by an
LLM (see llm/llm_interface.py), and mapped to lifestyle recommendations
(see recommendation/recommendation_engine.py).
